...
- Concatenated Supabase select strings infer `GenericStringError[]`. Keep `EXPENSE_LIST_SELECT` as one string literal so infinite-query pages flatten to expense rows. Activity merge items need `ExpenseListItem` / `TransferListItem` (including `group_id` and `title`) before they can render in `ExpenseItem`.
...
- `invite_tokens` has RLS. `anon` / `authenticated` have no table GRANT (no client-chosen token or expiry). Join resolves `join?token=` with `get_invite_group_id`, then `get_unbound_members_for_token` / `join_group_with_token` (all SECURITY DEFINER).
- Direct Data API inserts would still trust `token` and `expires_at` even with a membership INSERT policy, so minting stays on the RPC.
...
- Friends, account, notifications, language, profile, and create-group used to call `setSession(null)` on `isError`. That looks like a ban after a timeout. Show `FetchError` with Retry instead. Missing row uses "Profile not found".
- Explicit sign-out (`supabase.auth.signOut` on Settings) and account deletion still clear the session.
...
- Do not drain `useExpenseList` pages on Statistics or treat `hasNextPage` as a loading gate. That query is shared with group details (`staleTime` 0), so mount refetch + `fetchNextPage` can leave `hasNextPage` true and spin forever. Stats uses `get_group_expense_stats` (`["expenses", groupId, "stats", monthStart]`); the monthly header total uses `sum_group_expenses`. Neither downloads the expense list.
