...
const Member = ({ name, onRemove, owner }) => {
  return (
    ...
    >
      ...
      {!owner && (
        <TouchableOpacity className="absolute right-3" onPress={onRemove}>
  ...
  return (
    ...
    >
      <View className="flex-1 justify-center items-center bg-[rgba(0,0,0,0.5)]">
        <View
          className="absolute top-0 right-0 w-[70%] h-full bg-gray-300"
          style={{ gap: 20 }}
        >
          <View className="px-5">
            <Text className="text-2xl font-medium mt-12 mb-2.5">
              Add Participants
            </Text>
            <TextInput
              value={name}
          ...
          <TouchableOpacity
            className="flex-row items-center px-5 border rounded-lg p-1.5 mx-2.5 bg-green-600"
            style={{ gap: 10 }}
            onPress={addMember}
          >
            <Feather
          ...
          <View
            className="flex-row items-center p-2.5 bg-white absolute bottom-0 w-full"
            style={{ gap: 40 }}
          >
            <TouchableOpacity
              className="bg-transparent p-2.5 mb-4 flex-row items-center"
              onPress={onClose}
              style={{ gap: 5 }}
            >
